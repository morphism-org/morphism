...
const modulesPath = () =>
  Path.join(__dirname, "../../../node_modules/fp-ts-contrib/lib");
const outputPath = () => Path.join(__dirname, "../src/Generated.ts");

const shouldGenerate = (fileName: string): boolean =>
  fileName !== "index" &&
  fileName.charAt(0) === fileName.charAt(0).toUpperCase() && // Omit files that aren't domains
  !fileName.includes("Do") && // Omit Do to avoid collisions with our custom Do namespace
...
const generateCode = (moduleName: string, moduleSource: string): string[] => {
  ...
  const type = `export { ${reExport} } from "fp-ts-contrib/lib/${moduleName}";`;
  const lines = [
    `import * as ${capitalized}_ from "fp-ts-contrib/lib/${moduleName}";`,
